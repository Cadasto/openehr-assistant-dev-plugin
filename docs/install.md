...
This page is for maintainers setting up the plugin: installing it on Claude Code from the Cadasto marketplace, loading a working copy for development, updating it, adding it to Cursor, and pointing it at an MCP server. It ships for both [Claude Code](https://docs.claude.com/en/docs/claude-code/plugins) (`.claude-plugin/`) and [Cursor](https://cursor.com/docs/plugins) (`.cursor-plugin/`). Skill, agent, and rule content is shared; only the manifest and hook layer differ.

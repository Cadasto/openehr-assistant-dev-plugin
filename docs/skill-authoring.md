...
This page is for contributors adding or changing a skill or agent in this plugin: naming, layout, the `description` trigger pattern, the body, and the checks to run before committing. The plugin's whole job is authoring tooling, so its own skills and agents must be exemplary. It is the detailed companion to the "Authoring skills & agents here" section of [AGENTS.md](../AGENTS.md).
...
- **Skill / agent names**: terse activity nouns (for example `mcp-tool-authoring`, `example-authoring`, `release-workflow`, `repo-conventions-scout`). Skills are namespaced as `<plugin>:<skill>` (`openehr-assistant-dev:release-workflow`), so do not repeat the plugin's words in a skill name.
...
1. **What + scope**: one sentence: what the skill does, anchored to the target repo (for example "in the openehr-assistant-mcp repository"). This anchor *is* the scope limit.
...
3. **Anti-triggers**: a short "Not for …" / "Do NOT trigger …" that routes overlapping end-user cases to the owning plugin (for example "an end user wanting an AQL query → the user-facing `openehr-assistant` plugin").

The anti-trigger is the most important part for this plugin: every dev skill shares vocabulary with end-user openEHR work ("add an example", "write a guide"), so each must explicitly disclaim that context. Name the **strongest positive signal** too, which for most skills is "the workspace is the specific repo + the target file tree".

...
- **Skill / agent names**: terse activity nouns (for example `mcp-tool-authoring`, `example-authoring`, `release-workflow`, `repo-conventions-scout`). Skills are namespaced as `<plugin>:<skill>` (`openehr-assistant-dev:release-workflow`), so don't repeat the plugin's words in a skill name.

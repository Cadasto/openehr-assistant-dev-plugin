...
This page is for maintainers cutting a release of this plugin: how it applies [Semantic Versioning](https://semver.org) to skill and agent content, the release steps, and the marketplace update that makes a release live.
...
6. Cut the GitHub release from the tag, titled **exactly** the tag name, with the new CHANGELOG section as the body. Every tag in this org carries a release, and `-F -` reads the body from standard input:

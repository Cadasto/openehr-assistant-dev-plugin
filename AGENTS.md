...
This file provides guidance to AI coding assistants working in this repository. It is the canonical instruction source; host-specific files (`.github/copilot-instructions.md`, `rules/dev-context.mdc`, `.claude/CLAUDE.md`) delegate here.

## Project Overview

The **openEHR Assistant Dev Plugin** is a **maintainer-facing** AI plugin by Cadasto B.V. for Claude Code and Cursor. It is *not* an end-user clinical tool. It helps maintainers **design, implement, test, document, and release** MCP tools, prompts, resources, guides, completion providers, and examples for two sibling projects: it supplies the **authoring and release workflow layer** (which conventions apply where, how to add an artefact correctly, how to test it in the Docker dev container, and how to keep the two repos version-aligned). The human pitch lives in [README.md](README.md); do not restate it here.
...
|------|------------------|-----------------------|------------------------|
| [`openehr-assistant-mcp`](https://github.com/cadasto/openehr-assistant-mcp) | PHP 8.4, `modelcontextprotocol/php-sdk`, Docker | MCP tools (`#[McpTool]`), prompts (`#[McpPrompt]`), resources (`#[McpResourceTemplate]`/`#[McpResource]`), completion providers (`#[CompletionProvider]`), guides (`resources/guides/`), examples (`resources/examples/`), BMM, terminology | that repo's `AGENTS.md` |
| [`openehr-assistant-plugin`](https://github.com/cadasto/openehr-assistant-plugin) | Markdown + JSON (no build) | Plugin skills, commands, agents, hooks; dual-host Claude + Cursor manifests | that repo's `AGENTS.md` + `CONTRIBUTING.md` |

Release tags differ per repo: the MCP repo tags bare `X.Y.Z` (e.g. `0.20.0`); the plugin repo and this repo tag `vX.Y.Z`. Check `git tag` in the target before tagging.
...
- **Claude hooks**: `hooks/hooks.json`: object `{ "hooks": { "SessionStart": [ { "hooks": [ { "type": "command", ... } ] } ] } }`; use `${CLAUDE_PLUGIN_ROOT}` in command paths.
...
- **Validation**: `scripts/validate.sh` (graceful local wrapper: warns and skips if Python is absent) runs `scripts/validate.py`, which checks manifests, dual-host parity, declared paths, and frontmatter.
- **Docs**: `docs/` holds human-facing [install](docs/install.md), [testing](docs/testing.md), [versioning](docs/versioning.md), and [skill-authoring](docs/skill-authoring.md) references; `.github/` holds issue + PR templates, `copilot-instructions.md`, and the validate workflow.
...
|-------|-------------|---------|
| `guide-prompt-authoring` | mcp | Author implementation guides (`resources/guides/{category}/{name}.md`) and MCP prompts (`resources/prompts/{name}.md` + `src/Prompts/{Name}.php`). |
...
### Hooks
- **SessionStart**: detects which target repo the workspace is and prints the applicable skills and dev commands (for the MCP repo, the Docker-only reminder); silent in any other workspace.

## Development

### Testing & validating

Validate and dogfood locally (no build step: pure Markdown + JSON):
...
Then open one of the target repos and confirm the `SessionStart` hook detects it and the intended skills trigger, and that they stay silent in an end-user openEHR session. CI ([`.github/workflows/validate.yml`](.github/workflows/validate.yml)) runs `python3 scripts/validate.py` strictly on every push to `main` and every PR. Install, triggering tests, and release detail live in [`docs/`](docs/); the plugin installs as `openehr-assistant-dev@cadasto`.
...
### Documentation Sync
When adding or renaming components, update: **AGENTS.md** (component tables), **README.md** (tables), and **hooks/session-start.sh** (the skills list in its `dev` branch, plus the per-repo lists where the skill applies).

### Versioning
- Plugin version (and, for consistency, description and author) must stay in sync across **both** `.claude-plugin/plugin.json` and `.cursor-plugin/plugin.json`. Follow Semantic Versioning; update both manifests and `CHANGELOG.md` when releasing. The current version lives in the manifests and `CHANGELOG.md`, not here.
- **Release naming:** tags are annotated `vX.Y.Z`, and every tag gets a GitHub release titled exactly the tag name. Bump rules and release steps live in [docs/versioning.md](docs/versioning.md).
...
### Branching
- Feature branches and pull requests from `main`. CI validation runs on every PR and on every push to `main`.
...
- **A release tag here ships nothing on its own.** The `cadasto` marketplace pins this plugin to a release tag, so a release is not live until that entry's `version` and `source.ref` move; see [docs/versioning.md](docs/versioning.md#marketplace).
